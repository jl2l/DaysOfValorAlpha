...
public class BattleManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        objSP1.GetComponent<BattleSpawner>().enabled = true;
        objSP2.GetComponent<BattleSpawner>().enabled = true;
    ...
    public IEnumerator SearchPhase()
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < unitss.Count; i++)
            {
                ...
                if (go.GetComponent<DoV_LandUnit>().isReady)
                {
                    int alliance = go.GetComponent<DoV_LandUnit>().alliance;
                    if (go.GetComponent<DoV_LandUnit>().isAttackable == true)
                    {
                        if (go.GetComponent<DoV_LandUnit>().onTargetSearch == false)
                        {
                            runits[alliance].Add(go);
                            go.GetComponent<DoV_LandUnit>().onTargetSearch = true;
                        }
                    }

                    rfunits[alliance].Add(go);

                    go.GetComponent<DoV_LandUnit>().isReady = false;
                }
            }
            ...
            for (int i = 1; i < countr1; i++)
            {
                ...
                iorig1[i] = i;
                if (cprunits[1][i].GetComponent<DoV_LandUnit>().isAttackable == false)
                {
                    if (cprunits[1][i].GetComponent<DoV_LandUnit>().onTargetSearch == true)
                    {
                        cprunits[1][i].GetComponent<DoV_LandUnit>().onTargetSearch = false;
            ...
            for (int i = 1; i < countr2; i++)
            {
                ...
                iorig2[i] = i;
                if (cprunits[2][i].GetComponent<DoV_LandUnit>().isAttackable == false)
                {
                    if (cprunits[2][i].GetComponent<DoV_LandUnit>().onTargetSearch == true)
                    {
                        cprunits[2][i].GetComponent<DoV_LandUnit>().onTargetSearch = false;
            ...
            for (int i = 1; i < countrf1; i++)
            {
                ...
                tempObj1.GetComponent<DoV_LandUnit>().target = tempObj2;

                // adding attacker to target attackers list	
                tempObj2.GetComponent<DoV_LandUnit>().attackers.Add(tempObj1);
                tempObj2.GetComponent<DoV_LandUnit>().noAttackers = tempObj2.GetComponent<DoV_LandUnit>().noAttackers + 1;

                tempObj1.GetComponent<DoV_LandUnit>().isApproaching = true;
